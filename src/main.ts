...
// ============ Config Terapéutica ============
const Terapia: ConfigTerapia = {
  hemianopsia: false, cimt: false, saliencia: false,
  multi: true, enfoque: false, fotos: []
};

function guardarTerapia(): void {
  try { localStorage.setItem('rc-terapia', JSON.stringify(Terapia)); } catch (e) {}
}
...
function aplicarUITerapia(): void {
  ...
  $$('.opcion-terap').forEach(op => {
    const input = op.querySelector('input');
    op.classList.toggle('activa', input ? input.checked : false);
  });
...
function renderGaleria(): void {
  ...
  $$('.btn-borrar').forEach(b => {
    b.addEventListener('click', e => {
      ...
      Terapia.fotos.splice(idx, 1);
      guardarTerapia(); aplicarUITerapia();
    });
  });
}
...
const Sonido = {
  ...
  acierto(): void {
    ...
    if (Terapia.multi && this.voz) {
      const u = new SpeechSynthesisUtterance(aleatoria(['¡Muy bien Oscar!', '¡Excelente!', '¡Bravo!', '¡Sigue así!']));
      u.lang = 'es-AR'; u.rate = 0.9; u.pitch = 1.1;
  ...
  nivel(): void {
    ...
    if (Terapia.multi && this.voz) {
      const u = new SpeechSynthesisUtterance('¡Felicitaciones Oscar, terminaste el nivel!');
      u.lang = 'es-AR'; u.rate = 0.9; this.voz.cancel(); this.voz.speak(u);
    }
...
const Confetti = {
  ...
  inicializar(): void {
    this.canvas = $<HTMLCanvasElement>('#canvas-confetti');
  ...
  animar(): void {
    ...
    this.part.forEach(p => {
      ...
      this.ctx!.fillRect(-p.tam / 2, -p.tam / 4, p.tam, p.tam / 2);
      this.ctx!.restore();
    });
...
const Sesiones = {
  ...
  obtener(): Sesion[] {
    try { return JSON.parse(localStorage.getItem(this.clave) || '[]') as Sesion[]; } catch (e) { return []; }
  },
  exportar(): string {
...
const TITULOS: Record<JuegoId, string> = {
  memoria: '🧠 Memoria', atencion: '👀 Atención',
  coordinacion: '✋ Coordinación', viaje: '🚀 Viaje a las Estrellas'
};
const sigNivel = (n: Dificultad): Dificultad | null => {
  const i = NIVELES.indexOf(n);
  return i < NIVELES.length - 1 ? NIVELES[i + 1] : null;
};

const PANTALLAS: string[] = ['inicio', 'menu', 'memoria', 'atencion', 'coordinacion', 'viaje', 'progreso'];
function mostrarPantalla(n: string): void {
  PANTALLAS.forEach(p => $('#pantalla-' + p).classList.toggle('activa', p === n));
  const esJuego: boolean = ['memoria', 'atencion', 'coordinacion', 'viaje'].includes(n);
...
function mostrarResumen(datos: DatosResumen): void {
  ...
  Sesiones.registrar({
    juego: datos.juego, nivel: datos.nivel,
    puntaje: datos.puntaje + bonusCIMT, exitoso: datos.exitoso,
    stats: statsObj, tiempoIzq: datos.tiempoIzq ?? null, tiempoDer: datos.tiempoDer ?? null
  });
...
function alternarSonido(): void {
  ...
  ['#btn-sonido-inicio', '#btn-sonido-menu', '#btn-sonido-barra'].forEach(sel => {
...
function guiaVisual(container: HTMLElement): Promise<void> {
  return new Promise(resolve => {
    ...
    if (Terapia.multi && Sonido.voz) {
      const u = new SpeechSynthesisUtterance('Mirá a la izquierda');
      u.lang = 'es-AR'; u.rate = 0.8; Sonido.voz.cancel(); Sonido.voz.speak(u);
    }
...
const memState = {
  ...
  timer: null as number | null, sesion: 0
};
const Memoria: Juego = {
  async comenzar(n) {
    const s = memState;
    s.nivel = n || s.nivel; s.sesion++; Estado.terminado = false;
...
function coSolapa(x: number, y: number, t: number): boolean {
  return $$('#zona-coord .objetivo').some(o => {
    const ox = parseFloat(o.style.left), oy = parseFloat(o.style.top);
    return Math.hypot(ox - x, oy - y) < t * 1.1;
  });
}
...
function coVerificar(): void {
  const s = coState; const cfg = s.config[s.nivel];
  if (!s.finalizado && s.lanzados >= cfg.total && s.activos <= 0 && !s.spawner) {
    const ses = s.sesion; setTimeout(() => { if (s.sesion === ses) coFinalizar(); }, 500);
  }
}
...
function viajeInit(): void {
  ...
  document.addEventListener('keydown', e => {
    if (Estado.juegoActivo === 'viaje' && (e.code === 'Space' || e.code === 'ArrowUp')) { e.preventDefault(); if (s.corriendo && !Estado.enPausa) viajeSaltar(); }
  });
...
function viajeUpdate(dt: number): void {
  ...
  s.timerObs += dt;
  if (s.timerObs >= s.proxObs) {
    viajeCrearObstaculo(); s.timerObs = 0;
    s.proxObs = rand(cfg.obsMin, cfg.obsMax) * (cfg.velBase / s.velocidad);
  }
  s.timerEst += dt;
...
function viajeColisionar(): void {
  ...
  for (const st of s.estrellas) {
    ...
    if (dist < st.r + 28) {
      ...
      Sonido.estrella();
      if (Terapia.multi && Sonido.voz && s.estrellasRec % 5 === 0) {
        const u = new SpeechSynthesisUtterance('¡Estrella!'); u.lang = 'es-AR'; u.rate = 1; Sonido.voz.cancel(); Sonido.voz.speak(u);
      }
      viajeActualizarHUD();
    }
  }
...
const Juegos: Record<JuegoId, Juego> = { memoria: Memoria, atencion: Atencion, coordinacion: Coordinacion, viaje: Viaje };
...
// ============ MASCOTA ============
const CONSEJOS = [
  '💧 Tomá agua, ayuda al cerebro.', '🫁 Respirá profundo 3 veces.',
  '👀 Mirá bien a la izquierda al caminar.', '🤚 Mové los dedos de la mano izquierda.',
  '😴 Dormir bien es medicina.', '💊 Tomá la medicación a horario.',
  '🌞 Disfrutá la luz del día.', '✋ Estirá el brazo izquierdo suave.',
  '🧘 Sentate cómodo y respirá un minuto.', '💛 Cada paso cuenta. ¡Sos valiente!'
];
let _mascTimer: number | undefined;
function initMascota(): void {
  $('#mascota-bot').addEventListener('click', ev => {
    ev.stopPropagation(); Sonido.click();
    const b = $('#mascota-burbuja');
    b.textContent = '🐶 ' + aleatoria(CONSEJOS);
    b.classList.add('visible');
    clearTimeout(_mascTimer);
    _mascTimer = window.setTimeout(() => b.classList.remove('visible'), 6000);
  });
  document.addEventListener('click', ev => {
    if (!(ev.target as HTMLElement).closest('.mascota-contenedor')) $('#mascota-burbuja').classList.remove('visible');
  });
}
...
function init(): void {
  ...
  ['#btn-sonido-inicio', '#btn-sonido-menu', '#btn-sonido-barra'].forEach(sel => {
    const b = document.querySelector(sel); if (b) b.addEventListener('click', alternarSonido);
  });
  ...
  $('#btn-comenzar').addEventListener('click', () => { Sonido.click(); mostrarPantalla('menu'); });
  $('#btn-volver-inicio').addEventListener('click', () => { Sonido.click(); mostrarPantalla('inicio'); });
  ...
  $('#btn-ver-progreso').addEventListener('click', () => { Sonido.click(); mostrarPantalla('progreso'); });
  $('#btn-ver-progreso-menu').addEventListener('click', () => { Sonido.click(); mostrarPantalla('progreso'); });
  $('#btn-volver-desde-progreso').addEventListener('click', () => { Sonido.click(); mostrarPantalla('inicio'); });
